...
class _FloodPredictionPageState extends State<FloodPredictionPage_U> {
  ...
  final TextEditingController monsoonIntensityController = TextEditingController();
  final TextEditingController topographyDrainageController = TextEditingController();
  final TextEditingController riverManagementController = TextEditingController();
  ...
  final TextEditingController disasterPreparednessController = TextEditingController();
  final TextEditingController watershedsController = TextEditingController();
  final TextEditingController infrastructureController = TextEditingController();
  final TextEditingController populationScoreController = TextEditingController();
  final TextEditingController politicalFactorsController = TextEditingController();
  String? username;
  String? email;
  String? role;
...
Future<void> loadUsername() async {
  ...
  final String apiUrl = 'http://192.168.225.190:8000/predictf'; 

  Future<void> _predictFloodRisk() async {
    if (_formKey.currentState!.validate()) {
      // Gather and parse inputs as double
      final requestData = {
        "monsoon_intensity": double.tryParse(monsoonIntensityController.text),
        "topography_drainage": double.tryParse(topographyDrainageController.text),
        "river_management": double.tryParse(riverManagementController.text),
        "dams_quality": double.tryParse(damsQualityController.text),
        "siltation": double.tryParse(siltationController.text),
        "disaster_preparedness": double.tryParse(disasterPreparednessController.text),
        "watersheds": double.tryParse(watershedsController.text),
        "infrastructure": double.tryParse(infrastructureController.text),
        "population_score": double.tryParse(populationScoreController.text),
        "political_factors": double.tryParse(politicalFactorsController.text),
      };
      ...
      try {
        ...
        if (response.statusCode == 200) {
          ...
          final prediction = responseData['flood_probability'];
           ...
           if (username != null) {
          await DatabaseHelper.instance.insertPrediction(username!, 'Flood Prediction', prediction);
        } 
          _showPredictionDialog(prediction.toString());
        } else {
  ...
  void _showPredictionDialog(String prediction) {
    showDialog(
      ...
      builder: (context) {
        return AlertDialog(
          title: const Text('Flood Prediction Result'),
          content: Text('Flood Probability: $prediction'),
          actions: [
            TextButton(
                    ...
                    child: Padding(
                      ...
                      child: Column(
                        ...
                        children: [
                          ...
                          _buildInputField(monsoonIntensityController, 'Monsoon Intensity'),
                          _buildInputField(topographyDrainageController, 'Topography Drainage'),
                          _buildInputField(riverManagementController, 'River Management'),
                          _buildInputField(damsQualityController, 'Dams Quality'),
                          _buildInputField(siltationController, 'Siltation'),
                          _buildInputField(disasterPreparednessController, 'Ineffective Disaster Preparedness'),
                          _buildInputField(watershedsController, 'Watersheds'),
                          _buildInputField(infrastructureController, 'Deteriorating Infrastructure'),
                          _buildInputField(populationScoreController, 'Population Score'),
                          _buildInputField(politicalFactorsController, 'Political Factors'),
                          const SizedBox(height: 20),
                          ElevatedButton(
