...
def predict():
    ...
    for column in model_columns:
        if data is not None and column in data:
...
def predicte():
    ...
    try:
        ...
        numerical_features = {
            ...
            'tsunami': float(data.get('tsunami', 0)),
        ...
        alert = str(data.get('alert', '')).upper()
        net = str(data.get('net', '')).upper()

        # Encode categorical variables
        try:
            encoded_alert = label_encodere.transform([alert])[0] if alert else 0
            encoded_net = label_encodere.transform([net])[0] if net else 0
        except Exception as e:
            print(f"Label encoding error: {e}")
            return jsonify({"error": f"Invalid categorical value. Alert: {alert}, Net: {net}"}), 400

        # Prepare input array in correct order
        input_features = [
            numerical_features['cdi'],
            numerical_features['mmi'],
            encoded_alert,
            numerical_features['tsunami'],
            numerical_features['sig'],
            encoded_net,
            numerical_features['dmin'],
            numerical_features['gap'],
        ...
        prediction = modele.predict(input_array)
        magnitude = float(prediction[0])  # Convert numpy float to Python float
...
def emergency_request():
    try:
        ...
        msg = Message('Emergency Request Received',
                      ...
                      recipients=['sanalfleming1@gmail.com'])
        msg.body = f"""
